...
class StartPage(tk.Frame):
    def __init__(self, master):
        ...
        width=master.winfo_screenwidth()    ##Get Max Width Screen Size
        height=master.winfo_screenwidth()  ##Get Max Height Screen Size
        height=int(height/2)
        width=int(width-100)
        master.geometry(str(width)+'x'+str(height)+"+0+0")
        master.configure(bg="#b56969")
       

        tk.Label(master, text="ATTENDANCE USING FACE RECOGNITION",fg="#e8edf3",bg="#22264b",height=2,font=("Courier", 50),anchor="center").place_configure(x=0,y=0,relwidth=1)
        load = Image.open("mainpic.jpg")
        load = load.resize((750, 650), Image.ANTIALIAS)
        render = ImageTk.PhotoImage(load)

        img = tk.Label(self, image=render,anchor="center")
        img.image = render
        img.place_configure(relx=0.23, rely=0.1)
        def selection():
            a=radio.get()
        ...
        tk.Radiobutton(self, text="ADMIN",bg="#22264b",fg="#e8edf3",variable=radio, value=1,command=selection,width=10,height=1,font=("Courier", 45),selectcolor="pink").place_configure(relx=0.1,rely=0.85)
        tk.Radiobutton(self, text="FACULTY",bg="#22264b",fg="#e8edf3", variable=radio, value=2,command=selection,width=10,height=1,font=("Courier", 45)).place_configure(relx=0.4,rely=0.85) 
        tk.Radiobutton(self, text="STUDENT",bg="#22264b",fg="#e8edf3", variable=radio, value=3,command=selection,width=10,height=1,font=("Courier", 45)).place_configure(relx=0.7,rely=0.85) 
...
class PageOne(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="ADMIN LOGIN ACCOUNT",fg="#e8edf3",bg="#22264b",height=2,font=("Courier",50),anchor="center").place_configure(x=0,y=0,relwidth=1)
        tk.Label(self, text="Enter Username:",fg="#22264b",bg="#e8edf3",width=22,height=1,font=("Courier", 50),anchor="center").place_configure(relx=0.03,rely=0.3)
        e1 = tk.Entry(master,width=20,font=("Courier", 50))
        e1.pack()
        e1.place_configure(relx=0.53,rely=0.3)
        tk.Label(self, text="Enter Password:",fg="#22264b",bg="#e8edf3",width=22,height=1,font=("Courier", 50),anchor="center").place_configure(relx=0.03,rely=0.4)
        e2 = tk.Entry(master,width=20,font=("Courier", 50),show="*")
        e2.pack()
        e2.place_configure(relx=0.53,rely=0.4)
        tk.Button(self, text="LOGIN", command=lambda:callback4(e1.get(),e2.get()),relief=RAISED,width=20,fg="#22264b",bg='#e6cf8b',font=("Courier", 30)).place_configure(relx=0.7,rely=0.55,anchor="e",height=50,width=500)
        tk.Button(self,image=render,text="BACK",command=lambda: master.switch_frame(StartPage),height=80,width=90).place_configure(relx=0.0,rely=0.015)
