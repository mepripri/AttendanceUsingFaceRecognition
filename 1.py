...
"""class PageFour(tk.Frame):
    def __init__(self, master):
        ...
        OptionList=["Please Select Subject"]

        conn=sqlite3.connect("facedatabase.db")
        cursor=conn.cursor()
        cursor.execute("SELECT * FROM DataTea WHERE Id='"+id+"'")
        rows = cursor.fetchall()
        for row in rows:
            name1=row[3]
            OptionList.append(name1)
        conn.commit()
        cursor.close()
...
class PageFive(tk.Frame):
    def __init__(self, master):        
        ...
        tk.Button(self,image=render1,text="BACK",command=lambda: master.switch_frame(StartPage),height=80,width=90).place_configure(x=30,y=15)
...
class PageTwelve(tk.Frame):
    def __init__(self, master):
        ...
        def callback4(id,name,pwd,sub):
            ...
            for row in rows:
                id1=row[0]
            if(id=='' or name=='' or pwd=='' or id==id1 or sub=='' or sub=="Please Select Subject"):
                master.switch_frame(PageFifteen)
            else:
                conn=sqlite3.connect("facedatabase.db")
                cursor=conn.cursor()
                cursor.execute("INSERT INTO DataTea(Id,Name,Pass,Subject) VALUES('%s','%s','%s','%s')"%(id,name,pwd,sub))
                conn.commit()
                cursor.close()
        ...
        tk.Label(self, text="Enter Faculty Id:",fg="#22264b",bg="#e8edf3",width=20,height=1,font=("Courier", 50),anchor="center").place_configure(x=35,y=200)
        e1 = tk.Entry(master,width=20,font=("Courier", 50))
        e1.pack()
        e1.place(x = 685, y = 198)
        tk.Label(self, text="Enter Name:",fg="#22264b",bg="#e8edf3",width=20,height=1,font=("Courier", 50),anchor="center").place_configure(x=35,y=300)
        e3 = Entry(master,width=20,font=("Courier", 50))
        e3.pack()
        e3.place(x = 685, y = 298)
        tk.Label(self, text="Enter Password:",fg="#22264b",bg="#e8edf3",width=20,height=1,font=("Courier", 50),anchor="center").place_configure(x=35,y=400)
        e2 = tk.Entry(master,width=20,font=("Courier", 50),show="*")
        e2.pack()
        e2.place(x = 685, y = 398)
        tk.Label(self, text="Select Subject:",fg="#22264b",bg="#e8edf3",width=20,height=1,font=("Courier", 50),anchor="center").place_configure(x=35,y=500)
        ...
        tk.Button(self, text="SIGNUP", command=lambda :callback4(e1.get(),e3.get(),e2.get(),variable.get()),relief=RAISED,width=20,fg="#22264b",bg='#e6cf8b',font=("Courier", 30)).place_configure(x=1200,y=625,anchor="e",height=50,width=500)
...
class PageFifteen(tk.Frame):
    def __init__(self, master):
        ...
        def callback4(id,name,pwd,sub):
            ...
            for row in rows:
                id1=row[0]
            if(id=='' or name=='' or pwd=='' or id==id1 or sub==''):
                master.switch_frame(PageFifteen)
            else:
                conn=sqlite3.connect("facedatabase.db")
                cursor=conn.cursor()
